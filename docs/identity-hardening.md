# Learn identity hardening

Hara Learn accepts authenticated writes only through an audience-bound Identity handoff and a separate Learn-local session.
...
- Every Learn session and profile request rechecks that the community account remains active.
- Central sign-out uses a front-channel redirect through Learn so the host-only Learn cookie is cleared from any Hara site.
- `/.well-known/hara-learn-readiness` actively checks Identity, the Neon schema, GitHub App permissions, repository access, and the target branch.
...
3. Do not allow the Hara Learn GitHub App to bypass the ruleset.
...
The GitHub App remains installed only on `hara-lang/hara-learn` with Metadata read, Contents write, and Pull requests write.
