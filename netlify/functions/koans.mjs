...
import { clearWorldSessionCookie, json, readWorldSession, sameOrigin } from "./_shared/world-auth.mjs";
...
export async function handle(request, options = {}) {
  ...
  const identity = readWorldSession(request, env, options.now ?? Date.now());
  if (!identity) return error(401, "WORLD_SESSION_REQUIRED", "Sign in to save progress or reveal peer solutions.");
  let status;
  try { status = await (options.communityAccountStatusImpl ?? communityAccountStatus)(identity.id, options); }
  catch { return error(503, "WORLD_ACCOUNT_CHECK_FAILED", "World could not verify the community account."); }
  if (status !== "active") return error(403, "WORLD_ACCOUNT_INACTIVE", "This Hara World account is not active.", { "Set-Cookie": clearWorldSessionCookie(request.url) });
  ...
  if (!sameOrigin(request, env) || request.headers.get("x-hara-request") !== "koan-completion") {
    return error(403, "KOAN_REQUEST_REJECTED", "The completion must come from Hara World.");
  }
  let completion;
