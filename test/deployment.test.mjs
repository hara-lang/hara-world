...
test("World deployment verification proves Identity, proposal lifecycle readiness, and front-channel logout", async () => {
  ...
  assert.match(verifier, /My World/);
  assert.match(verifier, /Your proposal activity/);
  assert.match(verifier, /Review the World/);
  ...
  assert.doesNotMatch(verifier, /HARA_WORLD_GITHUB_WEBHOOK_SECRET/);
});
