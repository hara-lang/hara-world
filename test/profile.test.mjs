...
test("profile editing trusts only the Learn session and creates a reviewable Git change", async () => {
  ...
  assert.match(endpoint, /readLearnSession/);
  ...
  assert.doesNotMatch([accountPage, endpoint].join("\n"), /HARA_LEARN_GITHUB_APP_PRIVATE_KEY|HARA_LEARN_SESSION_SECRET\s*=/);
});
